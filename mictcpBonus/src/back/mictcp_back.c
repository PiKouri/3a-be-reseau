...
int mic_tcp_accept(int socket, mic_tcp_sock_addr* addr)
{
    ...
    sleep(500);
    while (!result) {
        result = (IP_recv(&syn, addr, TIMEOUT) != -1);
        result &= (syn.header.syn == 1) && (syn.header.ack == 0);
        if (result) {
            ...
            IP_send(synack, *addr);
            result = (IP_recv(&syn, addr, TIMEOUT) != -1);
            result &= (syn.header.syn == 0) && (syn.header.ack == 1);
